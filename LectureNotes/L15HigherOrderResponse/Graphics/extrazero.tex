\begin{tikzpicture}
\draw (0,0) node {
\begin{tikzpicture}[xscale=.3,yscale=.6]
\draw[->] (-11,0) -- (2,0) node[right=2pt] {Re$(s)$};
\draw[->] (0,-2) -- (0,2) node[above=2pt] {Im$(s)$};
\draw (-1,0) ++(0,.2) -- ++(0,-.4) node[below] {$-1$};
\draw (-10,0) ++(0,.2) -- ++(0,-.4) node[below] {$-10$};
\draw (-2,0) node {\Large\sf o};
\draw (-1,1.5) node {\Large\sf x};
\draw (-1,-1.5) node {\Large\sf x};
\end{tikzpicture}};

\draw (0,-3.5) node {\includegraphics[width=2in]{Graphics/extrazero1}};

\draw (6,0) node {
\begin{tikzpicture}[xscale=.3,yscale=.6]
\draw[->] (-11,0) -- (2,0) node[right=2pt] {Re$(s)$};
\draw[->] (0,-2) -- (0,2) node[above=2pt] {Im$(s)$};
\draw (-10,0) node {\Large\sf o};
\draw (-1,0) ++(0,.2) -- ++(0,-.4) node[below] {$-1$};
\draw (-10,0) ++(0,.2) -- ++(0,-.4) node[below] {$-10$};
\draw (-1,1.5) node {\Large\sf x};
\draw (-1,-1.5) node {\Large\sf x};
\end{tikzpicture}};

\draw (6,-3.5) node {\includegraphics[width=2in]{Graphics/extrazero2}};


\end{tikzpicture}