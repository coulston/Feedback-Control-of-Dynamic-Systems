...
\title[\GainandPhaseMarginBodeShortName] % (optional, use only with long paper titles)
{\course: \GainandPhaseMarginBodeName\license}

\subtitle
{Lecture \GainandPhaseMarginBodeNumber} % (optional)
...
\begin{itemize}
	\item \color{red}{this will be about 1/2 of the former Gain and Phase Margin article - the Bode half}
	\item \color{red}{all content is currently in this document, so will need to move some into the new GM and PM Nyquist lecture (number 33) - as I work through this one}
	\item \color{red}{also move QY and HW problems}
	\item \color{red}{update prerequisite material list}
		...
		\item \color{red}{ask questions like ``if you want to increase the gain (or phase) margin, what term(s) would you use?''}
\end{itemize}
...
We can check the closed-loop poles to see if any are in the right half-plane (RHP), where the closed-loop transfer function is given by
\[
T(s)=\frac{G(s)C(s)}{1+G(s)C(s)}
\]
As you'll recall, the closed-loop poles are the values of $s$ for which 
\[
1+C(s)G(s)=0 \rightarrow C(s)G(s)=-1
\]
Without (yet!\footnote{We'll explain a bit more in Lectures~\NyquistStabilityTheoremNumber-\NyquistStabilityAnalysisNumber}) getting into all of the details, we can see that there is something important about the point
\[
-1 = -1+j0 = 1 \angle-180^\circ
\]
that motivates the measures we introduce in this article: the gain margin and phase margin. In order to define these, we also need to define the \textit{crossover frequency}.

\begin{definition} The {\em crossover frequency} is the frequency $\omega_{co}$ at which the magnitude Bode plot passes through $0$~dB. 
	\end{definition}

\begin{definition} A {\em gain margin} (in dB) is the additional magnitude $\GM_{dB}$ that we can add to the magnitude Bode plot such that $20 \log_{10} G(j\omega_{180})+\GM_{dB}=0$~dB, where $\omega_{180}$ is the frequency at which the phase Bode plot crosses $-180^\circ$.
\end{definition}

\begin{definition} The {\em phase margin} is the phase lag $\PM$ such that the phase Bode plot would satisfy $\angle G(j\omega_{co})-\PM=-180^\circ$.  
\end{definition}


The gain and phase margins of a system are collectively referred to as the ``stability margins.'' The idea behind these is that we don't always know $G(s)$ exactly, which means we need some way to measure whether we are at high risk of the closed-loop system being unstable given that uncertainty.

%However, what if the plant $G(s)$ is uncertain? 
For example, the frequency response of $G(s)$ may have been determined from an experiment. Suppose a series of sinusoids of different frequencies were applied to $G(s)$, and we recorded the magnitude and phase shift of the output. The possible results of such an experiment are shown below in blue stars, with the green line indicating a plausible model for the resulting data. One issue with such an experiment is the loss of signal as the output magnitude decreases at high frequency. Usually a sensor has noise characteristics that are constant or perhaps increasing with higher frequency. If the signal we are measuring has lower magnitude at higher frequency, the signal to noise ratio decreases, and our estimates of the magnitude and phase are correspondingly poor. 
...
%Using the nominal model for $G(s)$ and taking $C(s)=1$, it might be difficult to know how much larger we can make $C(s)$ before the closed-loop system becomes unstable. 
%%the nominal Nyquist plot (shown in blue below) would indicate that the closed loop system is stable. However, if we use the worst case max and min phase and gain, shown by the cyan and red lines, an encirclement of $-1$ cannot be ruled out. In other words, we cannot guarantee closed-loop stability with this $C(s)$ due to the uncertainty in $G(s)$.
%%\begin{frame}
%%\begin{center}
...
As can be inferred by the name ``stability margin,'' the gain and phase margin play critical roles in the closed-loop stability. However, we will first be using them for controller design (Lectures~\TimeFrequencyNumber-\BodeControlDesignNumber) before introducing the Nyquist theory that we will use to analyze stability. In other words, we're defining and applying these terms now so that we can use some powerful control design tools, then will proceed to explain more about them in future lectures after introducing Nyquist theory. 

\subsection{Gain Margin}

To find the gain margin from a Bode plot of the loop gain $L(s) =C(s)G(s)$, we start by searching for the point on the Bode plot of $L(s)$ that has a phase of $-180^{\circ}$. We can refer to this frequency as $\omega_{180}$. %This point can be located using either the Bode plot of $L(s)$, or the Nyquist plot. Using the Bode plot, we follow the following steps:

\begin{itemize}
\item Locate $\omega_{180}$ where the Bode plot crosses the $-180^{\circ}$ phase line (including any shifts by a multiple of $360^{\circ}$).
...
To find the phase margin, we search for the point on the Bode plot that has a magnitude of 1, which corresponds to $0~$dB and is known as the crossover frequency $\omega_{co}$. This point will be the one that crosses $-1+j0$, but this time when the phase is changed. Since only the phase is changed, and $-1+j0$ has a magnitude of $1$, the point we are looking for also has a magnitude of $1$. This point can be found using the following procedure.

\begin{itemize}
\item Locate the crossover frequency $\omega_{co}$ where the Bode plot crosses the $0$dB magnitude line.
...
\begin{minipage}{4.7in}
\begin{tikzpicture}
\draw (-6.4,0) node {\includegraphics[]{figures/marginsexample1_5}};
