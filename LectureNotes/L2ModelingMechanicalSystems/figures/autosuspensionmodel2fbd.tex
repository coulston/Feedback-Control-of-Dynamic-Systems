\begin{tikzpicture}

\draw[inner sep=0pt,outer sep=0pt,very thick] (2,0) node[rotate=90] (K1) {\input{\mainfolder/DrawingElements/MechanicalElements/spring.tex}};
\draw (K1) node[right=14pt] {$k$};
\draw[inner sep=0pt,outer sep=0pt,very thick] (-2,0) node[rotate=90] (D1) {\input{\mainfolder/DrawingElements/MechanicalElements/damper.tex}};
\draw (D1) node[left=14pt] {$b$};
\draw (0,3) node[draw,rectangle,minimum width=1.5cm,minimum height=1.5cm,very thick] (M1) {$m_{c}$};

\draw[|->] (K1.180) ++(-.35,-.25) node[left] {$x_{k1}$} -- ++(0,.5);
\draw[|->] (K1.0) ++(-.35,.25) node[left] {$x_{k2}$} -- ++(0,.5);
\draw[|->] (D1.180) ++(.35,-.25) node[right] {$x_{b1}$} -- ++(0,.5);
\draw[|->] (D1.0) ++(.35,.25) node[right] {$x_{b2}$} -- ++(0,.5);
\draw[|->] (M1) ++(1.25,0) node[right] {$x_{c}$} -- ++(0,.5);

\draw[very thick] (K1.180) -- ++(0,-.25) ;
\draw[very thick] (D1.180) -- ++(0,-.25) ;
\draw[very thick] (K1.0) -- ++(0,.25) ;
\draw[very thick] (D1.0) -- ++(0,.25) ;

\draw[<-] (M1.90) -- ++(0,.5) node[right] {$m_{c}g$};
\draw[<-] (K1.180) ++(0,-.25) -- ++(0,-.5) node[below] {$f_{k}$};
\draw[<-] (K1.0) ++(0,.25) -- ++(0,.5) node[above] {$f_{k}$};
\draw[<-] (D1.180) ++(0,-.25) -- ++(0,-.5) node[below] {$f_{b}$};
\draw[<-] (D1.0) ++(0,.25) -- ++(0,.5) node[above] {$f_{b}$};

\draw[<-] (M1.-90)  -- ++(0,-.5) node[below] {$f_{b/k}$};
\end{tikzpicture}
