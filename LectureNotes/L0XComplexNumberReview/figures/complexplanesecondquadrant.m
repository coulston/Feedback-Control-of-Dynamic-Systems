...
line([-.5 -.5],[.03 -.03])
line([.03 -.03],[.7 .7])
text(-.52,-.06,'\fontname{times}\fontsize{14}\ita')
text(.05,.7,'\fontname{times}\fontsize{14}\itjb')
text(-.63,.7,'\fontname{times}\fontsize{14}\its')
line([0 -.5],[0 .7])
text(.1,.2,'\fontname{times}\fontsize{14}\it\theta')
text(-.25,.5,'\fontname{times}\fontsize{14}\itr')
theta=0:.01:(pi-atan(.7/.5));
plot(.1*cos(theta),.1*sin(theta),'k-')
axis equal
print -dpdf -f1 complexplanesecondquadrant.pdf