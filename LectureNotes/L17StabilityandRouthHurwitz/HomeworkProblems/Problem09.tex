Consider the following hydraulic control system. The operating point is $\bar{x}=0$, $\bar{y}=0$, and  the linearized servo valve equation is $\delta q = 2 \delta x - 4 \delta p$. A controller is implemented with transfer function $\frac{K}{s+1}$. Find the values  of $K$ for which the closed loop system is stable.
\begin{center}
...
% control
\draw (-.5,1) node[sysblock]  (K) {$\frac{K}{s+1}$};
