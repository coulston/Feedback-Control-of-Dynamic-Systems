...
\draw (8,0) node[sysblock] (G) {$\frac{1}{s^{2}+3s}$};
...
%\draw[->] (sum3.0) -- node[above=2pt,pos=.4] {$X(s)$}  (sum2.-180) node[above left=2pt] {$+$};
%\draw[->] (sum2.0) -- (G);
\draw[->] (sum3.0) -- node[above=2pt,pos=.5] {$V_{a}(s)$}  (G.180);
\draw[->] (G) -- ++(2.5,0) node[above=2pt] {$\theta(s)$};
