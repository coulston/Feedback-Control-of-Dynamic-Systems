...
\draw (-4,3) node {\textsf{Domain of Nyquist Plot}};
\draw[->] (-4,-2) -- ++(0,4) node[left] {Im$\{s\}$};
\draw[->] (-5,0) -- ++(2,0) node[below] {Re$\{s\}$};
\draw[->,very thick,color=blue] (-4,-1.5) -- (-4,-.5); 
\draw[->,very thick,color=blue] (-4,-.5) -- (-4,.5); 
...
\draw (-4.5,-2.5) node[right] {$\rho=$ big enough number so that $L(j \rho)$ approaches a constant};
