In some robotics applications, it is desired to control the torque applied to a load, rather than the position or velocity. Given the following DC motor with inertial load, find the transfer function with input $V_{a}$ and output $\tau$. Hint: re-arrange the block diagram by shifting blocks or stretching lines (without changing the fundamental relationships) so that $\tau$ is the signal on the right, then simplify the block diagram.
\begin{center}
...
\draw (3.5,0) node {$J$} ++(0,-.6) node {\small arm};
...
\draw (5,0) node (K) {\input{\mainfolder/DrawingElements/MechanicalElements/spring.tex}};
