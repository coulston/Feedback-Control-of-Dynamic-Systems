...
\draw[very thick] (2,-1) -- ++(1,.5) -- ++(0,-2) -- ++(-1,.5);
...
\draw[<-] (4,1) -- ++(0,.5) node[right] {$f$};

% load
\draw (7,-1) node[draw,very thick,above,dotted,rectangle,minimum width=.65in,minimum height=.4in] (L) {Load};

\draw[<-] (L.-90) -- ++(0,-.5) node[right] {$f$};
\draw[|->] (L.-90) ++(1,0) node[right] {$y$} -- ++(0,.5);
