Find an equivalent circuit for the following fluid system. The pressure on the left and right, $p_{in,1}$ and $p_{in,2}$ are inputs.   The fluid density is $\rho$.  Label $p_{1}$, $p_{2}$, and $p_{a}$ in the circuit, and use gauge pressure, so that all pressures are specified with respect to atmospheric pressure.  
\begin{center}
\begin{tikzpicture}
...
\draw[very thick] (7.1,.5) rectangle ++(1,1);
...
\draw (pipe0.180) node[left] {$p_{in,1}$};
\draw (pipe3.0) node[right,rectangle,draw=white,fill=white] {\rule{0pt}{.25in}\rule{.25in}{0pt}};
\draw (pipe3.0) node[right] {$p_{in,2}$};
